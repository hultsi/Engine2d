...
namespace Engine2d {
	...
	void collision::getCollisionNormal(Rectangle &rect_1, Rectangle &rect_2, std::unique_ptr<Vector2d> &normal_1,
		...
		for (int i = 0; i < N; ++i) {
			for (int k = 0; k < maxProjs; ++k) {
				...
				if ((*d0_Min > *d1_Max || *d1_Min > *d0_Max)) {
					...
					if (r1 == &rect_1) {
						normal_1->x = sign(*d1_Min - *d0_Min) * proj.x;
						normal_1->y = sign(*d1_Min - *d0_Min) * proj.y;
					} else {
						normal_1->x = -sign(*d1_Min - *d0_Min) * proj.x;
						normal_1->y = -sign(*d1_Min - *d0_Min) * proj.y;
					}
